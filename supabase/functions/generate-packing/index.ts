...
const SYSTEM_PROMPT = `You are Zo, ZYPH's travel copilot. Generate a practical packing list for ONE trip.

Return ONLY JSON of the exact shape:
{"items":[{"label": string, "category": one of ["clothes","toiletries","documents","electronics","health","other"], "quantity": integer >= 1}]}

Rules:
- Tailor the list to the DESTINATION, TRIP LENGTH and WEATHER provided.
- 10 to 25 concise items, no duplicates, realistic quantities (e.g. tops ~ trip length).
- "label" is short (1-4 words), no brand names, no explanations.
- Output JSON only, no markdown, no commentary.
- Write the labels in the language given by LANGUAGE (fr = French, en = English).`
...
export default {
  fetch: withSupabase({ auth: ["publishable"] }, async (req) => {
    ...
    let body: { destination?: unknown; days?: unknown; weather?: unknown; language?: unknown }
    try {
      body = await req.json()
    } catch {
...
WEATHER: ${weather || "unknown"}`
    ...
    const rawItems = Array.isArray((parsed as { items?: unknown })?.items)
      ...
      : []
    const items: { label: string; category: string; quantity: number }[] = []
    for (const raw of rawItems) {
      const item = raw as { label?: unknown; category?: unknown; quantity?: unknown }
      ...
      const quantity =
        typeof item.quantity === "number" && Number.isFinite(item.quantity)
          ...
          : 1
      if (label) {
        items.push({ label, category, quantity })
      }
