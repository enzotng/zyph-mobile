...
// Self-contained Balances body: owns its own loading/error/empty/loaded states, FlashList,
// BottomSheet, and the share/settle-all affordances in-body (no parent Screen `right`), so it
// can be dropped into both the Spend tab's in-place segment and the `/balances` route wrapper.
export function BalancesView({ tripId }: { tripId: string }) {
  ...
  const ListHeader = (
    <View style={styles.headerSections}>
      {/* Top affordance row: share sits in-body (not a parent Screen `right`) so this view stays
          self-contained across both the Spend tab's in-place segment and the /balances route. */}
